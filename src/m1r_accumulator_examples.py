...
#
#            for k in range(r - 1):
#            for k in range(r): This will run it r times
#            for k in range(r + 1):
#
...
#
#              for k in range(r - s):   add one for it to be inclusive between r and s
#                  count = 0    count should be outside of for loop or else count will be set to 0 every time it runs,
#                  if math.cos(r) > 0:    r is a constant integer, it should be r + k
#                      count = 1        the count should be summing up the cos(r+k) so count = count + cos(r+k)
#                  return count     the return function should be outside of the if function
#
#     f. The code in the "graphics accumulation" example below includes:
#            for _ in range(n):
#        What does the   _   (underscore) mean?   It is the variable that increases each time the loop
#
...
#
#              x = starting_point.x   <--------This one is the wrong one
#              for k in range(n):
